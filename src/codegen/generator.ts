 ...
 */
export function generate(manifest: Manifest, sourceMapping?: SourceMapping): GeneratedOutput {
  const sm = sourceMapping ?? defaultSourceMapping(manifest);
  const plans = planAll(manifest, sm);

  const loadDataSQL = generateLoadData(manifest, sm);
...
function generateLoadData(manifest: Manifest, sm: SourceMapping): string {
  ...
  for (const entity of manifest.entities) {
    ...
    lines.push(
      ...
      `SELECT * FROM read_csv_auto('data/university/${es.table}.csv');\n`
    );
  }
  for (const rel of manifest.relationships) {
    ...
    lines.push(
      ...
      `SELECT * FROM read_csv_auto('data/university/${rs.table}.csv');\n`
    );
  }
  return lines.join("\n");
}
