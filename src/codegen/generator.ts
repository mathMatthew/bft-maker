...
export interface GenerateOptions {
  ...
  dataDir?: string;
  /**
   * Where generated transformations read their source tables.
   *
   * CSV mode preserves the original behavior: emit load SQL and run against
   * an in-memory DuckDB database. Database mode assumes the source tables
   * already exist and emits a runner that takes the DuckDB path as argv[1].
   * Default: "csv".
   */
  source?: "csv" | "database";
}
...
export function generate(manifest: Manifest, options?: GenerateOptions): GeneratedOutput {
  ...
  const loadDataSQL = source === "csv" ? generateLoadData(manifest, sm, dataDir) : "";
  const tables = plans.map((plan) => ({
    name: plan.tableName,
    sql: generateTableSQL(plan, manifest, sm),
  }));
  const runScript = generateRunScript(tables.map((t) => t.name), source);
...
function generateRunScript(tableNames: string[], source: "csv" | "database"): string {
  // Build file list with numbered prefixes matching emitFiles output
  const sqlFiles = [
    ...(source === "csv" ? ["00_load_data.sql"] : []),
  ...
  // Preserve the original no-argument, in-memory runner for CSV sources.
  if (source === "csv") {
...
`;
  ...
  const tableNamesPy = JSON.stringify(tableNames);

  // Database-backed runners take the path at execution time. Keeping the path
  // out of generated SQL and shell source avoids quoting bugs and makes the
  // generated directory portable between database copies.
  return `#!/bin/bash
# Generated by bft-maker
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DB_PATH=\${1:?"Usage: run.sh <database_path>"}

echo "Running BFT SQL..."
python3 - "$SCRIPT_DIR" "$DB_PATH" <<'PYEOF'
import duckdb, os, sys
...
for table_name in table_names:
    ...
    print(f'Materialized {table_name}: {row_count} rows')
con.close()
print()
if all_pass:
    print('All validations passed.')
